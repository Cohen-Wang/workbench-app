          ...
          <view v-for="(item, index) of userList" :key="index" class="cu-item">
            ...
            <view class="action">
              ...
              <checkbox class="round blue"
                        :checked="item.singleChecked"
                        @click="onUserSelect(index)"/>
            </view>
          </view>
        </view>
  ...
  methods: {
    show(roleId) {
      ...
      if (!this.userList.length) {
        this.loading = true
        apiService.getManagerList(this.roleId).then(res => {
          this.userList = res.data.result.map(e => {
    ...
    onUserSelect(index) {
      this.userList = this.userList.map((e, i) => {
        e.singleChecked = i === index
        return e
      })
    ...
    confirmUserSelection() {
      ...
      const theOne = this.userList.find(e => e.singleChecked)
      if (!theOne) return
      this.$emit('onConfirm', theOne)
    }
  }
}
</script>
